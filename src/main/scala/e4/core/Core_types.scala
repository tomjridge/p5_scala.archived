...
object Core_types {
  ...
  trait Earley_state_t { self: Symbol_t with Item_t =>
    ...
    def state_to_k(s:State) : k_t
    def todo_is_empty(x:State) : Boolean
  ...
  abstract class All_traits extends Substring_t with Symbol_t with Item_t
    with Input_t with Grammar_t with Ctxt_t with Earley_state_t
  {
    // some impl code we need in core, but don't want to give because they expose State type
    def inc_k(s0:State) : State
