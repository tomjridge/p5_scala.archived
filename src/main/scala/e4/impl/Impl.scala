...
object Impl {
	...
	{   
		...
		def sym_case(x:sym) = (x:sym_ab)
		def sym_of_tm(x:tm) = TM(x)
	  ...
	  def find_ktjs(x:tm,s0:State) : Option[List[Int]] = {
		...
		def init_state(c0:Ctxt,nt:nt) :State = {
			val (i,k) = (0,0)
			val init = (nt,i,List(),k,List(NT(nt)))
			val todo = List(init)
	...
	def main(args:Array[String]) {
		...
		def p_of_tm(tm:tm) = {
			if (TM(tm) == eps) parse_eps
			else if (TM(tm) == x) parse_x
		...
		def nt_items_for_nt(nt:nt,si:(string_t,Int)) = {
			val (s,i) = si
			val as = List()
			val k = i
			List(
		...
		def do_work = {
			println("Start")
			val r = fg_earley(c,e2)
			println("Finished")
    }
		//do_work
    Timing.time(10,do_work)
    // length 100, Elapsed time: 21951158535 ns (21.951158535 secs) (avg: 2.1951158535)
    // this seems very slow, given that the ocaml version does **200** in less than 1s 
    // and scala e3 does it in 0.5s :(
